...
impl fmt::Display for PackableError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PackableError::Io => write!(f, "I/O error happened."),
            PackableError::InvalidVariant => write!(f, "Invalid variant read."),
            PackableError::InvalidUtf8String => write!(f, "Invalid Utf8 string read."),
            PackableError::InvalidVersion => write!(f, "Invalid version read."),
            PackableError::InvalidType => write!(f, "Invalid type read."),
            PackableError::InvalidAnnouncedLen => write!(f, "Invalid announced len."),
        }
    }
}
